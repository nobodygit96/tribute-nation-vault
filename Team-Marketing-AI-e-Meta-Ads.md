...
- **Consolidamento deciso da Vale il 2026-08-25**: la situazione non era ordinata (due account pubblicitari con storie separate, uno migrato in ritardo, un pixel relitto isolato, nessun collegamento diretto Instagram↔account pubblicitario). Deciso di usare **`2429131157229173` come account pubblicitario canonico** da qui in avanti; `1548998116564966` resta storico, non più operativo. **Azione eseguita**: verificato che collegare risorse in Business Settings è reversibile (stesso pattern del dataset, pulsante "Scollega" sempre presente, nessun avviso di irreversibilità — a differenza della migrazione di un intero account pubblicitario), poi collegato esplicitamente `@_tribute_nation_` a `2429131157229173` tramite "Risorse collegate" → ora **2 risorse collegate all'account Instagram**: Pagina Facebook TributeNation + account pubblicitario `2429131157229173`. Non risolve da solo il "sembra nuovo" nell'app (quella scelta resta lato app, non nel grafo BM) ma è il primo passo concreto di consolidamento, e rende esplicito qui quello che prima non lo era.
...
- [ ] Decidere se/quando sistemare il profilo WhatsApp Business (refuso nel nome "Trubute Nation", azienda non verificata) — non urgente, il numero funziona comunque
