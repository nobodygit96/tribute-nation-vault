...
- **Consolidamento (2026-08-25)**: deciso `2429131157229173` come account pubblicitario canonico da qui in avanti. Collegato esplicitamente a `@_tribute_nation_` tramite "Risorse collegate" (verificata prima la reversibilità: pulsante "Scollega" sempre disponibile, nessun avviso di irreversibilità, a differenza della migrazione di un intero account pubblicitario). L'account Instagram ora ha 2 risorse collegate: Pagina Facebook TributeNation + questo account pubblicitario.
- **Account WhatsApp — esiste e funziona, verificato il 2026-08-25**: "Trubute Nation" (refuso nel nome, non corretto — non toccato), di proprietà del BM, ID `1665804671431720`. Numero reale collegato e attivo: **+39 392 891 2701** (Italia), stato "Collegato", qualità "Alta". Profilo business incompleto (non verificata, nessun indirizzo/valuta/metodo di pagamento) ma il numero funziona. Nessuna scheda "Risorse collegate" per gli account WhatsApp in questa UI — non partecipano al grafo asset di Pagine/Instagram/account pubblicitari/dataset.
...
- **Installazione sul sito**: codice base inserito in un Code Snippet ("TikTok - Pixel base (tracciamento)", ID snippet 110) su tributenation.it, stesso meccanismo già usato per il Pixel Meta (hook `wp_head`).
- **Gate di consenso + attivazione — fatto il 2026-09-10**: lo snippet 110 era rimasto inattivo perché, a differenza del Pixel Meta, non aveva nessun blocco di consenso (avrebbe tracciato tutti i visitatori da subito, prima del banner). Aggiunto un gate diretto nello stesso snippet (non uno snippet separato come per Meta, perché qui il codice base è scritto a mano, non un plugin di terzi): usa l'API di consenso nativa del pixel (`ttq.holdConsent()`/`grantConsent()`/`revokeConsent()`, sintassi ufficiale TikTok "default opt-out", da business-api.tiktok.com/portal/docs/pixel-cookie-consent-mode), stesso purpose IAB TCF 4 già usato e confermato da Daniele per il gate Meta (TN-155). Hook a priorità 2, deliberatamente dopo lo Snippet #6 (iubenda Cookie Banner, priorità 1) cosi' che `window.__tcfapi` sia già definito quando il gate lo controlla. Passato da `tn-deploy-reviewer` prima dell'attivazione. **Snippet ora Attivo.** Verificato: nessun tag TikTok duplicato in GTM (solo GA4 e iubenda), pixel resta "held" per i nuovi visitatori senza consenso (0 eventi/PageView registrati su TikTok Events Manager anche dopo più caricamenti), consenso già presente in questo browser correttamente riconosciuto (`purpose4: true`) con chiamate pixel regolari. La dashboard TikTok segnala fino a 30 minuti di ritardo prima di mostrare i primi eventi: da ricontrollare più avanti per la conferma finale lato TikTok.
