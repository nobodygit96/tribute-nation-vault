...
- **Serve Daniele**: accesso al pannello iubenda.com (progetto siteId `4577327`) per controllare le impostazioni di "blocking mode"/script blocking e la mappatura delle categorie di consenso, per permettere il caricamento di GTM stesso (lasciando che sia il tag Consent Mode v2 dentro GTM a gestire il consenso granulare dei singoli tag, come già configurato). **Task da aprire sul Monitor non appena raggiungibile** (era offline durante questa sessione, `192.168.1.87:4173` irraggiungibile).
...
**Prossimo passo per riprendere**: task da aprire per Daniele (Monitor irraggiungibile in questa sessione) per sbloccare il caricamento di GTM lato iubenda — è l'unico vero blocco rimasto, GA4/GTM/Search Console sono tutti configurati correttamente ma GA4 non riceve ancora dati reali per questo motivo.
